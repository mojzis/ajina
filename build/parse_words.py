...
def preserve_existing_images(
    words: list[dict[str, str]], existing_path: Path
) -> list[dict[str, str]]:
    """Re-use image filenames from a prior run of the same week.

    Image filenames have a random suffix generated by parse_line, so re-parsing
    a week would otherwise reshuffle every filename and orphan the previously
    generated webp files. For each word whose id already exists in the prior
    JSON, copy the old `image` value over.
    """
    ...
    try:
        ...
        print(f"  WARNING: could not read existing {existing_path.name}: {e}", file=sys.stderr)
        return words

    prior_images: dict[str, str] = {
        w["id"]: w["image"] for w in prior.get("words", []) if "id" in w and "image" in w
    }

    preserved = 0
    for word in words:
        if word["id"] in prior_images:
            word["image"] = prior_images[word["id"]]
            preserved += 1

    if preserved:
        print(f"  Preserved {preserved} existing image filename(s) from prior run")
...
def main() -> None:
    ...
    # Preserve image filenames from any prior run of this week, so existing
    # generated images aren't orphaned by the random-suffix reshuffle.
    week_path = data_dir / f"week-{args.week}.json"
    words = preserve_existing_images(words, week_path)
